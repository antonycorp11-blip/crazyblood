...
export type Save={blood:number;district:number;unlocked:number;era:number;relics:number;progress:Record<number,number>;bestByCity:Record<number,number>;missions:Record<number,boolean>;cleared:Record<number,boolean>;levels:Record<string,number>;nights:number;total:number;best:number;victory:boolean;muted:boolean}
const KEY='hemofarm-cacada-v4'
const defaultSave=():Save=>({blood:0,district:0,unlocked:0,era:0,relics:0,progress:{},bestByCity:{},missions:{},cleared:{},levels:{},nights:0,total:0,best:0,victory:false,muted:false})
export const load=():Save=>{try{const x=JSON.parse(localStorage.getItem(KEY)||'null');if(x&&typeof x.blood==='number')return {...defaultSave(),...x,levels:x.levels||{},progress:x.progress||{},bestByCity:x.bestByCity||{},missions:x.missions||{},cleared:x.cleared||{}}}catch{}return defaultSave()}
...
export class Night{
  ...
  constructor(save:Save){
    ...
    for(let i=0;i<10+this.district.city*8+this.district.era*3;i++)this.spawn()
  }
  get district(){return DISTRICTS[this.save.district]}
  get requiredCaptures(){return this.save.district===19?1000:this.district.quota}
  get horizontalBounds():[number,number]{
    if(typeof window==='undefined'||window.innerWidth>650||window.innerHeight<=window.innerWidth)return [140,860]
    const half=Math.min(340,window.innerWidth/(window.innerHeight-64)*280)
    return [500-half+20,500+half-20]
  }
  ...
  spawn(named=false){
    ...
    const x=rand(minX+10,maxX-10),y=rand(165,470)
  ...
  update(dt:number){
    ...
    for(const h of this.humans){
      ...
      h.x=clamp(h.x,minX,maxX);h.y=clamp(h.y,153,480)
    ...
    this.animations=this.animations.filter(a=>a.age<a.duration)
  ...
  click(x:number,y:number,radius=28){
    ...
    if(!target){this.alarm=Math.min(100,this.alarm+.8);return false}
  ...
  hit(h:Human,damage:number,helper:boolean){
    ...
    if(!helper&&(h.kind==='hunter'||h.named&&this.district.targetType==='hunter')&&h.attack<=0){
      this.health=Math.max(0,this.health-1);h.attack=2.3;this.shakes=6;this.vampireHurt=.5;this.alarm=Math.min(100,this.alarm+7);this.burst(h.x,h.y-26,'#ffd174',8,'-1 ♥');this.lastSound='hurt'
    }
  ...
  finish(reason:'dawn'|'defeat'|'lockdown'|'retreat'='retreat'){
    ...
    if(reason==='dawn'&&this.health>0&&this.missionThisNight&&this.captures>=this.requiredCaptures){
      this.qualified=true
